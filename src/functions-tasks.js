...
function getPolynom(...rest) {
  function setParam(x) {
    return rest.reduce(
      (acc, item, index) => acc + item * x ** (rest.length - 1 - index),
      0
    );
  }
  return setParam;
...
function logger(func, logFunc) {
  function getRes(...args) {
    const argRes = args.map((item) => JSON.stringify(item)).join(',');
    logFunc(`${func.name}(${argRes}) starts`);
    const res = func(...args);
    logFunc(`${func.name}(${argRes}) ends`);
    return res;
  }
  return getRes;
}
...
function getIdGeneratorFunction(startFrom) {
  let startFromCopy = startFrom - 1;
  function getId() {
    startFromCopy += 1;
    return startFromCopy;
  }
  return getId;
}
